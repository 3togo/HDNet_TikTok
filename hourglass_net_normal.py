...
def conv_layer(input_tensor,name,kernel_size,output_channels,initializer=tf.keras.initializers.variance_scaling,stride=1,bn=False,training=False,relu=True):
...
def batch_norm_layer(input_tensor,scope,training):
	#return tf.keras.layers.BatchNormalization(name="BatchNorm",scope=scope,is_training=training,decay=0.99)(input_tensor)
	return tf.keras.layers.BatchNormalization(name="BatchNorm")(input_tensor)
